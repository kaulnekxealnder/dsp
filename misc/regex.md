...
`\|` | `A \| B`, where A and B can be arbitrary REs, creates a REGEX that will match either A or B.  These can be used inside groups.  As scanned RE's are tried left to right.  Once one pattern completely matches, that branch is accepted and no further testing is completed | 
